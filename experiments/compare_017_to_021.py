"""
compare_017_to_021.py
---------------------
Overlay plot comparing exp_017 (GRPO baseline) with exp_018-021.

Usage: python compare_017_to_021.py [--out DIR]
Saves to: experiments/figures_comparison/all_experiments.png
"""
import re
import os
import argparse
...
def main(out_dir: str):
    ...
    fig.suptitle(
        "exp_017 → exp_021 · GRPO variants on Big-Math (integer) · Llama-3.2-3B · 16 gens · bs=4 · bf16",
    ...
    fig.suptitle("exp_017 → exp_021 · Reward & KL", fontsize=12, fontweight="bold")
    for short, exp_label, color, records in experiments:
        if not records:
            continue
        steps = [r["step"] for r in records]
