"""
gtpo_ema_v2_trainer.py
----------------------
Fixed GTPO-EMA trainer for exp_010.

Key fix: token advantages computed directly from raw rewards (not pre-normalized
seq_advantages), so EMA confidence bonus is preserved and not washed out.
"""
import torch
from trl import GRPOTrainer
from .ema_confidence_utils_v2 import confidence_from_logits, compute_gtpo_ema_advantages, EPS


class GTPOEMAv2Trainer(GRPOTrainer):
    """
    GTPO-EMA v2: token-level EMA confidence advantage shaping.

    Kwargs:
        alpha1 (float): weight for base GRPO advantage (default 1.0)
        alpha2 (float): weight for EMA confidence bonus (default 0.1)
    ...
    def _compute_loss(self, model, inputs):
        ...
        # Raw rewards (before GRPO normalization) — key fix vs exp_006
        # TRL stores them in inputs["rewards"] (list of per-reward-func tensors summed)
        # Fall back to seq_advantages if raw rewards not available
        raw_rewards = inputs.get("rewards")
        if raw_rewards is not None and isinstance(raw_rewards, torch.Tensor):
            seq_rewards = raw_rewards
        else:
            # Fallback: use pre-normalized advantages (less ideal but works)
        ...
        # Compute confidence from current model logits
        with torch.no_grad():
        ...
        # Compute token-level advantages with EMA confidence
