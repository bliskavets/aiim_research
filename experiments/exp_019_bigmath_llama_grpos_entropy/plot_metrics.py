"""
Plot training metrics for exp_019 (GRPO-S entropy).
"""
import re
import os
import argparse
...
COLOR = "tab:green"
EXP_NAME = "exp_019"
EXP_LABEL = "exp_019 GRPO-S"
CUSTOM_KEYS = [
    ("grpo_s/mean_seq_is_weight", "Mean Seq IS Weight"),
    ("grpo_s/mean_seq_entropy",   "Mean Seq Entropy"),
    ("grpo_s/frac_pos",           "Fraction O+ Sequences"),
]
CUSTOM_TITLE = "exp_019 — GRPO-S Internal Metrics"
...
def plot_custom(records, out_path, color=COLOR):
    steps = [r["step"] for r in records]
...
def main():
    ...
    if not rec:
        print("No records in exp_019 log.")
        return
    print(f"Parsed {len(rec)} steps from {LOG_PATH}")
    summary = quick_summary(rec)
    print(f"  Peak reward: {summary['peak_reward']:.3f} @ step {summary['peak_step']}")
    print(f"  Latest reward: {summary['latest']}")
    with open(os.path.join(OUT_DIR, "metrics_summary.json"), "w") as f:
        json.dump(summary, f, indent=2)

    title = (
        "exp_019 — GRPO-S entropy · Llama-3.2-3B · Big-Math (integer) · "
        "16 gens · batch=4 · bf16"
    )
    plot_dashboard(rec, os.path.join(OUT_DIR, "dashboard.png"), title)
    plot_custom(rec, os.path.join(OUT_DIR, "grpo_s_metrics.png"))

    rec_017 = parse_log(LOG_017)
    if rec_017:
        plot_comparison(rec, rec_017, os.path.join(OUT_DIR, "compare_017_vs_019.png"))
