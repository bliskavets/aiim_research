"""Plot training metrics for exp_021 (GTPO-Conf, no EMA)."""
import re
import os
import json
import numpy as np
...
COLOR = "tab:purple"
EXP_LABEL = "exp_021 GTPO-Conf"
CUSTOM_KEYS = [
    ("gtpo_conf/mean_confidence",      "Mean Confidence (C_i,t)"),
    ("gtpo_conf/mean_token_advantage", "Mean Token Advantage"),
    ("gtpo_conf/frac_pos",             "Fraction O+ Sequences"),
]
CUSTOM_TITLE = "exp_021 — GTPO-Conf Internal Metrics"
...
def main():
    ...
    if not rec:
        print("No records in exp_021 log (not started yet).")
        return
    ...
    title = "exp_021 — GTPO-Conf · Llama-3.2-3B · Big-Math (integer) · 16 gens · bf16"
    plot_dashboard(rec, os.path.join(OUT_DIR, "dashboard.png"), title)
    plot_custom(rec, os.path.join(OUT_DIR, "gtpo_conf_metrics.png"))

    rec_017 = parse_log(LOG_017)
    if rec_017:
        plot_comparison(rec, rec_017, os.path.join(OUT_DIR, "compare_017_vs_021.png"))
