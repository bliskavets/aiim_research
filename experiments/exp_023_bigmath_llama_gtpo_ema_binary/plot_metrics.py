"""
Plot training metrics for exp_023 (GTPO-EMA binary).
...
Generates:
  ...
  figures/gtpo_ema_metrics.png — GTPO-EMA binary specific metrics
...
def plot_dashboard(records: list[dict], out_path: str, title: str):
    ...
    for gs_loc, key, label in panels:
        ...
        ax.plot(steps, smooth(vals), color=color, linewidth=2, label="exp_023")
...
def plot_reward_detail(records: list[dict], out_path: str):
    ...
    fig.suptitle("exp_023 — GTPO-EMA binary · Reward Functions Detail", fontsize=13, fontweight="bold")
...
# ─────────────────────────────────────────────────────────────────────────────
# GTPO-EMA binary specific metrics
# ─────────────────────────────────────────────────────────────────────────────

def plot_gtpo_ema_metrics(records: list[dict], out_path: str):
    steps = [r["step"] for r in records]
    fig, axes = plt.subplots(1, 3, figsize=(16, 5))
    fig.suptitle("exp_023 — GTPO-EMA binary Internal Metrics", fontsize=13, fontweight="bold")

    panels = [
        ("gtpo_ema_binary/mean_confidence",      "Mean Confidence (C_i,t)"),
        ("gtpo_ema_binary/mean_token_advantage", "Mean Token Advantage"),
        ("gtpo_ema_binary/frac_pos",             "Fraction O+ Sequences"),
    ]
...
# ─────────────────────────────────────────────────────────────────────────────
# Comparison: exp_017 vs exp_023
# ─────────────────────────────────────────────────────────────────────────────

def plot_comparison(rec018: list[dict], rec017: list[dict], out_path: str):
    fig = plt.figure(figsize=(18, 10))
    fig.suptitle(
        "exp_017 (GRPO baseline) vs exp_023 (GTPO-EMA binary) — Big-Math · Llama-3.2-3B · 16 gens",
    ...
    for gs_loc, key, label in panels:
        ...
        for records, color, name in [
            ...
            (rec018, "tab:orange", "exp_023 GTPO-EMA binary"),
        ]:
            if not records:
                continue
            steps = [r["step"] for r in records]
...
def main(log_path: str = LOG_PATH, out_dir: str = OUT_DIR, log_017: str = LOG_017):
    ...
    if not rec018:
        print("No records found in exp_023 log — training may not have started yet.")
        return
    ...
    title = (
        "exp_023 — GTPO-EMA binary · Llama-3.2-3B · Big-Math (integer) · "
    ...
    if rec017:
        ...
        plot_comparison(rec018, rec017, os.path.join(out_dir, "compare_017_vs_023.png"))
    else:
