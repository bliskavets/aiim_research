"""
exp_025: Pure-proof GTPO-EMA on GSM8K.

Same training config as exp_005 / exp_024 (GSM8K, Llama-3.2-3B, 500 steps,
lr=5e-6, bs=1, grad_accum=4, 4 generations, max_seq=2048). Only the trainer
differs: GTPOEMAProofTrainer implements Def 1.4 of GTPO-EMA-full.txt
(per-token EMA bonus with Σ-conservation, not z-normalized separately).
"""
import re, os, sys
...
TRAINING_CONFIG = {
    ...
    "output_dir": "/workspace/outputs_exp025_gtpo_ema_proof",
}
# α₁+α₂=1 for Prop 2.3 conservation; λ=0.9 gives Var(EMA)≈0.053·Var(C).
GTPO_EMA_PROOF_CONFIG = {"alpha1": 0.9, "alpha2": 0.1, "lam": 0.9,
                         "top_k": 20, "reward_threshold": 0.0}
...
from src.gtpo_ema_proof_trainer import GTPOEMAProofTrainer
...
def main():
    print("=== Exp 025: Pure-proof GTPO-EMA on GSM8K ===")
    dataset = prepare_dataset()
    print(f"Dataset: {len(dataset)} examples")
    model, tokenizer = FastLanguageModel.from_pretrained(
        **{k:v for k,v in MODEL_CONFIG.items() if k!='lora_rank'},
