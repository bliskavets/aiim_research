"""
plot_progress.py — quick dashboard of exp_027 training progress.
Reads train.log, parses all `{'loss': ...}` records, plots reward / KL /
...
def main():
    ...
    title = f"exp_027 — GRPO baseline on Big-Math int-2000 — {len(r)}/500 steps"
    ...
    for ax, key, label, is_kl in panels:
        ...
        ax.plot(steps, smooth(vals), color="tab:blue", linewidth=2.0, label=key)
        if key == "reward":
    ...
    ax.plot(steps, smooth(rewards), color="tab:blue", linewidth=2.4, label="reward (smoothed, w=15)")
    ax.axhline(9.5, color="red", linestyle=":", alpha=0.5, label="ceiling 9.5")
