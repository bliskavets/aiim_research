"""
plot_progress.py — exp_039 training dashboard.
...
EXP_TAG  = "exp_039 — GTPO-EMA-flipped · Qwen3-4B · Big-Math int-2000"
...
def main():
    ...
    for ax, key, label in panels:
        ...
        ax.plot(steps, vals, color="tab:orange", alpha=0.2, linewidth=0.6)
        ax.plot(steps, smooth(vals), color="tab:orange", linewidth=2.0)
        if key == "reward":
    ...
    ax.plot(steps, rewards, color="tab:orange", alpha=0.25, linewidth=0.8)
    ax.plot(steps, smooth(rewards), color="tab:orange", linewidth=2.4, label="reward (smoothed w=20)")
    ax.axhline(9.5, color="red", linestyle=":", alpha=0.5, label="ceiling 9.5")
