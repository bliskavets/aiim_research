"""
exp_039: GTPO-EMA-flipped — Qwen3-4B on Big-Math int-2000.
Same dataset/hyperparams as exp_038 (GRPO baseline). Trainer from exp_035.
max_seq_length=4096, batch_size=4, num_generations=8, max_steps=1000.
"""
import re, os, sys
...
TRAINING_CONFIG = {
    ...
    "output_dir": "/workspace/outputs_exp039",
...
GTPO_EMA_FLIPPED_CONFIG = {"alpha1": 0.9, "alpha2": 0.1, "lam": 0.9,
...
def reward_answer_exact(prompts, completions, answer, **kwargs):
    ...
    for guess, true_answer in zip(extracted, answer):
        ...
        if guess == true_answer: scores.append(3.0)
            ...
            except (ValueError, ZeroDivisionError): scores.append(-1.5)
    return scores
...
def main():
    print("=== Exp 039: GTPO-EMA-flipped Qwen3-4B on Big-Math int-2000 (bs=4, gens=8, 1000 steps) ===")
    ...
    trainer = GTPOEMAFlippedTrainer(
        ...
        **GTPO_EMA_FLIPPED_CONFIG,
    )

    print(f"Starting GTPO-EMA-Flipped: {TRAINING_CONFIG['num_generations']} gens × bs={TRAINING_CONFIG['per_device_train_batch_size']} = {TRAINING_CONFIG['num_generations']*TRAINING_CONFIG['per_device_train_batch_size']} seqs/step")
    trainer.train()
