"""
plot_progress.py — exp_040 training dashboard.
...
EXP_TAG  = "exp_040 — GTPO-EMA-flipped+RolloutLog · Qwen3-4B · Big-Math int-2000"
...
def main():
    ...
    for ax, key, label in panels:
        ...
        ax.plot(steps, smooth(vals), color="tab:green", linewidth=2.0)
        if key == "reward":
    ...
    ax.plot(steps, smooth(rewards), color="tab:green", linewidth=2.4, label="reward (smoothed w=20)")
    ax.axhline(9.5, color="red", linestyle=":", alpha=0.5, label="ceiling 9.5")
