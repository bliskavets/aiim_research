"""
exp_043: GTPO-Conf — Qwen3-4B on MATH benchmark (levels 3-5, integer answers).

Same dataset and reward functions as exp_041 (GRPO baseline) and exp_042
(GTPO-EMA-flipped ablation), but uses column-wise confidence-based per-token
advantages from exp_005 (which beat GRPO on GSM8K):

  O+: reward uncertain tokens more — log(1+C) / sum_col, z-normalized
  O-: penalize confident tokens more — log(1+1/C) / sum_col, z-normalized

compute_loss is overridden directly (not _compute_loss) to bypass unsloth's
patched GRPOTrainer.
...
from src import GTPoConfRolloutTrainer
...
TRAINING_CONFIG = {
    ...
    "output_dir": "/workspace/outputs_exp043",
...
def main():
    ...
    print("=== Exp 043: GTPO-Conf, Qwen3-4B, MATH levels 3-5 integer answers ===")
    print(f"GTPO-Conf config: {GTPOCONF_CONFIG}")
    ...
    trainer = GTPoConfRolloutTrainer(
