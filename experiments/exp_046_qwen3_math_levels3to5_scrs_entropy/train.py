"""
exp_046: SCRS-Entropy — Sequence-Level Shannon Entropy Reward Shaping, Qwen3-4B, MATH levels 3-5.

Like SCRS (exp_045) but uses top-k Shannon entropy H instead of top-k confidence C:

    shaped_adv_i = grpo_adv_i + alpha2 * entropy_adj_i

where entropy_adj_i = z-normalised mean top-k entropy of rollout i.

  - High entropy (uncertain) + correct  → bigger advantage  (hard win, amplify)
  - Low  entropy (confident) + wrong    → bigger penalty     (confident mistake)
  - alpha2=0 recovers pure GRPO exactly.

Comparison with exp_045 (SCRS-Confidence): sign is + instead of −
because H ≈ inverse of C (high H ↔ low C).
"""
import re, os, sys
...
from src import SCRSEntropyTrainer
...
TRAINING_CONFIG = {
    ...
    "output_dir": "/workspace/outputs_exp046",
...
def main():
    ...
    print("=== Exp 046: SCRS-Entropy, Qwen3-4B, MATH levels 3-5 integer answers ===")
    print(f"SCRS-Entropy config: {SCRS_ENTROPY_CONFIG}")
