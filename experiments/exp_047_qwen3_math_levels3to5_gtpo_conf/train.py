"""
exp_047: GTPO-Conf — per-token confidence reward shaping, Qwen3-4B, MATH levels 3-5.

Ported from exp_005 (GSM8K, Llama-3.2-3B) to MATH levels 3-5 (Qwen3-4B).
Key improvements over exp_005:
  - Fully vectorised per-token loops (no Python for-t overhead)
  - TRL 0.23.1 API (compute_loss, not _compute_loss)
  - Ghost guard for zero-variance batches
  - Rollout logging (.npz) for viewer analysis

Method:
  C_i,t = -mean_{v∈top-k}(log π_v)  — large = uncertain
  O+ (correct): bonus    ∝ log(1 + C)    → reward uncertain/exploratory tokens
  O- (wrong):   penalty  ∝ log(1 + 1/C)  → punish confident mistakes
"""
import re, os, sys
...
from src import GTPOConfTrainer
...
TRAINING_CONFIG = {
    ...
    "output_dir": "/workspace/outputs_exp047",
...
def extract_boxed(solution):
    ...
    for i, c in enumerate(solution[start:]):
        if c == '{':   depth += 1
            ...
            if depth == 0: return solution[start:start + i].strip()
            depth -= 1
    return None
...
def main():
    ...
    print("=== Exp 047: GTPO-Conf, Qwen3-4B, MATH levels 3-5 integer answers ===")
    print(f"GTPO-Conf config: {GTPO_CONF_CONFIG}")
