"""
plot_metrics.py — 4-way comparison for exp_049.

Parses train_<method>.log for each method and overlays total reward, answer_exact,
format_exact and KL. Mirrors the metric-extraction recipe used across the repo.
...
def main():
    ...
    fig.suptitle("exp_049 — Big-Math int-2000, Llama-3.2-3B · GRPO vs 3 candidates "
                 "(identical hyperparameters, seed 3407)", fontsize=13, weight="bold")
    ...
    out = os.path.join(HERE, "figures", "exp049_4way_comparison.png")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    fig.tight_layout()
    fig.savefig(out, dpi=130)
    print(f"saved {out}")

    # last-50 mean summary
    print("\nlast-50 mean reward:")
