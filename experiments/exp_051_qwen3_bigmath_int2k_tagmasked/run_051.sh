#!/usr/bin/env bash
# run_051.sh — exp_051: Qwen3-4B port of exp_050 (tagmasked shaping, full reward).
...
EXP_NAME="exp_051_qwen3_bigmath_int2k_tagmasked"
EXP_DIR="${REPO_ROOT}/experiments/${EXP_NAME}"
HF_TOKEN="${HF_TOKEN:?HF_TOKEN env var not set}"

echo "=== [$(date -Is)] Pre-flight: wiping exp_051's own previous artefacts ==="
sudo rm -rf \
...
echo ""
echo "=== [$(date -Is)] Launching exp_051 (4 methods, sequential) ==="
