"""
exp_051 — Qwen3-4B port of exp_050: GRPO vs 3 tag-masked shaped candidates on Big-Math int-2000
================================================================================================

Same setup as exp_050 but with Qwen/Qwen3-4B in place of Llama-3.2-3B.
max_seq_length raised to 4096 (canonical for Qwen3 thinking-mode completions,
per exp_038/039/040). GRPO baseline runs without tag mask; the 3 shaped
candidates (grpo_s_entropy, gtpo_conf, gtpo_ema_flipped) get format_tag_patterns
passed in so per-token shaping is masked off on the 4 structural tag
substrings (n/a for grpo_s_entropy since its shaping is seq-level).
...
def main():
    ...
    output_dir = f"/workspace/exp_051_qwen3_bigmath_int2k_tagmasked/outputs_{method}"
