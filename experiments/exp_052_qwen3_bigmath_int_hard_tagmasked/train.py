"""
exp_052 — exp_051 on a harder Big-Math subset (Llama-8B solve_rate < 0.3)
==========================================================================

Same setup as exp_051 (Qwen3-4B, max_seq=4096, full reward, tag-masked
shaping in the per-token methods), but the dataset is further filtered
to keep only problems where Llama-8B's solve rate is below 0.3 — i.e.
problems that are hard for the reference 8B model. The hope is that on
harder problems the Qwen3 baseline has more room above its weak-baseline
floor, so shaped methods have more headroom to differ from baseline
(exp_051 saw a near-ceiling regime where the per-token shaping helped
gtpo_conf slightly and hurt gtpo_ema_flipped).
...
MODEL_CONFIG = {
    ...
    "max_seq_length": 4096,          # 512 prompt + 3584 completion (exp_038/039/040 budget for Qwen3)
    "lora_rank": 64,
    "load_in_4bit": False,
    "fast_inference": True,
    "gpu_memory_utilization": 0.55,  # leave room for full-vocab logits forward in gtpo_conf
}
...
# =============================================================================
# DATASET — Big-Math, integer-answer ∩ Llama-8B-hard subset (exp_052)
# =============================================================================
...
SOLVE_RATE_THRESHOLD = 0.3   # keep only problems where Llama-8B solves <30% of rollouts
...
def main():
    ...
    output_dir = f"/workspace/exp_052_qwen3_bigmath_int_hard_tagmasked/outputs_{method}"
