#!/usr/bin/env bash
# run_054_resume_ema.sh — gtpo_ema_flipped only.
# exp_054 grpo was stopped at step ~460 (user call — reward plateaued
# at L50 ≈ +3.63 since step 350). train_grpo.log is preserved; this
# script runs ONLY gtpo_ema_flipped so we can collect a comparable
# masked-shaping trajectory without redoing grpo.
...
echo "=== [$(date -Is)] Pre-flight: wiping ONLY gtpo_ema_flipped artefacts (keep grpo log intact) ==="
sudo rm -rf \
...
echo ""
echo "=== [$(date -Is)] Launching exp_054 gtpo_ema_flipped only ==="
docker run --rm --gpus all \
  ...
  " 2>&1 | tee "${EXP_DIR}/run_054_resume.out"
...
echo ""
echo "=== [$(date -Is)] exp_054 gtpo_ema_flipped COMPLETE ==="
