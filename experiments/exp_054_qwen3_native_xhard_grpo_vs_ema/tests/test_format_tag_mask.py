...
# ─── exp_054: Qwen3 native special tokens only (no custom format tags) ───────
#
...
def test_train_script_passes_qwen3_native_tag_strings_to_encode_tag_patterns():
    """Static AST check: train.py for exp_054 must call encode_tag_patterns
    ...
    for node in ast.walk(tree):
        if (isinstance(node, ast.Call)
            ...
            # exp_054 must use ONLY Qwen3 native tags. assert presence...
            ...
            for old in ("REASONING_START", "REASONING_END",
                ...
                assert old not in literals, (
                    f"old custom tag {old} should not be in exp_054 mask; "
...
def test_train_script_does_not_reference_custom_format_tag_identifiers():
    """Static AST check: train.py for exp_054 must NOT define REASONING_START
    ...
    for forbidden in ("REASONING_START", "REASONING_END",
        ...
        assert forbidden not in module_level_names, (
            f"{forbidden} still defined at module level in exp_054 train.py")
