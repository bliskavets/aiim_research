#!/usr/bin/env bash
# run_055.sh — exp_055: Qwen3-4B GRPO + 3 candidates with tag-masked shaping on
# Big-Math int-2000 (= exp_051 slice, no llama8b filter), Qwen3 native format from exp_054.
# All 4 methods: grpo + 3 shaped (tag-mask
...
echo "=== [$(date -Is)] Pre-flight: wiping exp_055's own previous artefacts ==="
sudo rm -rf \
...
echo ""
echo "=== [$(date -Is)] Launching exp_055 (4 methods: grpo + 3 shaped, sequential) ==="
