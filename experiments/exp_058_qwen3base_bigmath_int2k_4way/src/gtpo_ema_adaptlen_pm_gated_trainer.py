"""
gtpo_ema_adaptlen_pm_gated_trainer.py
-------------------------------------
exp_058 method #6: the PER-POLARITY adaptive length penalty of method #5 (own knee
L_+/L_- within each group's O+ / O- subgroups), GATED by the same low-temperature
difficulty heuristic as the other gated trainers.
...
apply the per-polarity adaptive penalty to that prompt's training completions;
otherwise we skip it. Robust fallback: on any gate failure the penalty stays
ungated (= method #5), flagged via a metric.
"""
import torch
...
class GTPOEMAFlippedAdaptLenPMGatedTrainer(GRPOTrainer):
    ...
    def _generate_and_score_completions(self, inputs):
        ...
        n_comp = cm.shape[0]
        device = cm.device
        # per-polarity adaptive group-relative length penalty
        lengths = cm.sum(dim=1).float()
        adv = out["advantages"].detach().float().reshape(-1)
        ...
        try:
            ...
            if not self._gate_warned:
                print(f"[gtpo_ema_adaptlen_pm_gated] WARN gate disabled (ungated fallback): {e!r}", flush=True)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        self._metrics[mode].setdefault("gtpo_ema_adaptlen_pm_gated/mean_len", []).append(
            self.accelerator.gather(completion_mask.sum(dim=1).float().mean()).mean().item())
        self._metrics[mode].setdefault("gtpo_ema_adaptlen_pm_gated/pen_rel_absmean", []).append(
            self.accelerator.gather(pen_rel.abs().mean()).mean().item())
        self._metrics[mode].setdefault("gtpo_ema_adaptlen_pm_gated/pen_present", []).append(pen_present)
        ...
        if adapt_L is not None and adapt_L.shape[0] == B:
            ...
            if finite.any():
                self._metrics[mode].setdefault("gtpo_ema_adaptlen_pm_gated/mean_L", []).append(
                    self.accelerator.gather(aL[finite].mean()).mean().item())
        if gate is not None and gate.shape[0] == B:
            self._metrics[mode].setdefault("gtpo_ema_adaptlen_pm_gated/gate_frac", []).append(
