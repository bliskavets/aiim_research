"""
exp_059 — Qwen3-4B-Instruct, 4 methods on HARD Big-Math (llama8b_solve_rate < 0.3)
======================================================================================

The exp_051/052 recipe (grpo + 3 shaped candidates on SynthLabsAI/Big-Math-RL-Verified,
integer-answer ∩ llama8b_solve_rate<0.3, first 2000) with the FIXED (non-bypassed)
shaped trainers.

Goal: RE-VALIDATE the core shaping claim on the instruct model WITH HEADROOM. The
whole exp_049-055 arc claimed shaping helps on instruct Big-Math, but (a) the
shaping was silently bypassed there (ran plain GRPO — see SHAPING_BYPASS_BUGFIX),
and (b) the easy Big-Math slice saturates Qwen3-instruct (exp_055 null). This fills
the missing cell: instruct + hard subset (non-saturated) + shaping that ACTUALLY
runs. Complements exp_058 (base model, easy full-2000) and exp_056 (Search-R1).
...
MODEL_CONFIG = {
    "model_name": "Qwen/Qwen3-4B",   # exp_059: INSTRUCT (re-validate shaping on instruct w/ headroom)
    "max_seq_length": 4096,          # exp_051/052 config (512 prompt + 3584 completion)
    "lora_rank": 64,
    "load_in_4bit": False,
    "fast_inference": True,
...
# exp_059: hard subset — keep only problems Llama-8B solves <30% of the time
# (gives the INSTRUCT Qwen3 headroom; the easy slice saturates — exp_055). Same
# threshold as exp_052. llama8b_solve_rate is stored as a string.
SOLVE_RATE_THRESHOLD = 0.3
...
def is_integer_and_hard(example: dict) -> bool:
    return is_integer_answer(example) and is_llama8b_hard(example)
...
def prepare_dataset():
    ...
    n0 = len(ds)
    ds = ds.filter(is_integer_and_hard)
    n1 = len(ds)
    print(f"Filter: total={n0} -> integer-answer AND llama8b_solve_rate<{SOLVE_RATE_THRESHOLD}={n1}")
    ...
    print(f"Dataset: {len(ds)} hard integer-answer examples "
          f"(Big-Math llama8b<{SOLVE_RATE_THRESHOLD}, shuffled seed={DATASET_CONFIG['shuffle_seed']})")
    return ds
...
def main():
    ...
    print(f"=== exp_059 [{method}] — Big-Math HARD (llama8b<0.3), Qwen3-4B-Instruct (tagmasked shaping, FIXED non-bypassed) ===")
