"""
plot_progress.py — intermediate progress figure for exp_060 (run while training).
...
Saves figures/exp060_progress.png. Honest about being mid-run: the title shows
...
def main():
    ...
    fig.suptitle(
        f"exp_060 — Qwen3-4B on Omni-MATH integer subset (1971), Qwen3 native format · IN PROGRESS [{status}]",
        fontsize=12, weight="bold")
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    out = os.path.join(HERE, "figures", "exp060_progress.png")
