"""
exp_060 — CONTROL: GRPO-S with beta2=0 vs GRPO (does the GRPO-S code reduce to GRPO?)
======================================================================================

Same setup as exp_057 (Qwen3-4B, Omni-MATH integer subset, Qwen3 native format,
identical hyperparameters), but a SANITY CONTROL for the suspicion that even
grpo_s_entropy underperformed GRPO in exp_057.

We set the GRPO-S entropy-bonus weight beta2=0. Then compute_grpo_s_rewards
collapses to shaped = +beta1 for O+ (reward>0), -beta1 for O- — a sign-binarized,
group-normalized advantage = "GRPO with binarized rewards". It is NOT identical to
GRPO's continuous advantage, but it MUST learn ~like GRPO if the GRPO-S injection
and gradient path are correct. So we run two methods — `grpo` and
`grpo_s_entropy` (beta2=0) — and compare:
  * if grpo_s(beta2=0) tracks grpo -> the GRPO-S code/grad path is sound, and the
    entropy shaping (beta2>0) is what dragged it down in exp_057;
  * if grpo_s(beta2=0) diverges from grpo -> there is a bug in the grpo_s path.

Everything except beta2 (and the methods run) is identical to exp_057:
...
TRAINING_CONFIG = {
    ...
    "max_steps": 500,                   # exp_055: 2x more updates vs exp_052
    "save_steps": 9999,
    "max_grad_norm": 1.0,
    "report_to": "none",
    "seed": SEED,
...
SHAPING_CONFIG = {
    # exp_060 CONTROL: beta2=0 disables the entropy bonus -> grpo_s reduces to a
    # sign-binarized group-normalized advantage ("GRPO with binarized rewards").
    "grpo_s_entropy":   {"beta1": 1.0, "beta2": 0.0, "reward_threshold": 0.0},
