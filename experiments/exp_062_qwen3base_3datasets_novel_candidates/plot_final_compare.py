"""
exp_062 — final overlay across all 4 setups: GRPO, gtpo_ema_flipped(FIXED) at
λ∈{0.5,0.7,0.9}, and pos_discount. 4 columns (datasets) x 2 rows (answer_boxed,
completion length), rolling-30.
"""
import os, re
...
# (log-suffix, label, color)  — suffix relative to train_<ds>_
CURVES = [
    ("grpo",                        "GRPO",             "#64748b"),
    ("lam0.5",                      "FIXED λ=0.5",      "#93c5fd"),
    ("lam0.7",                      "FIXED λ=0.7",      "#2563eb"),
    ("gtpo_ema_flipped_fixed",      "FIXED λ=0.9",      "#1e3a8a"),
...
for j, (ds, ds_lab) in enumerate(DATASETS):
    ...
    for suf, lab, c in CURVES:
        ...
        lwd = 2.6 if lab == "pos_discount" else 2.0
        if bx:
            ys = rolling(bx); axb.plot(range(len(ys)), ys, color=c, lw=lwd, label=f"{lab} ({lm(bx):+.2f})")
        if cl:
            ys = rolling(cl); axl.plot(range(len(ys)), ys, color=c, lw=lwd, label=f"{lab} ({lm(cl):.0f})")
        if bx or cl:
...
fig.suptitle("exp_062 — GRPO vs flipped FIXED (λ=0.5/0.7/0.9) vs pos_discount across all setups (Qwen3-4B-Base, 300 steps)\n"
             "top=answer_boxed reward, bottom=completion length.",
             fontsize=11, weight="bold")
out = os.path.join(HERE, "figures", "exp062_final_compare.png")
os.makedirs(os.path.dirname(out), exist_ok=True)
fig.tight_layout(rect=[0, 0, 1, 0.93]); fig.savefig(out, dpi=140)
print(f"saved {out}")
print(f"\n{'dataset':10s} {'method':14s} {'L50_len':>9s} {'L50_boxed':>10s} {'steps':>6s}")
for ds, m, ml, mb, n in rows:
    print(f"{ds:10s} {m:14s} {ml:>9.0f} {mb:>+10.2f} {n:>6d}")
