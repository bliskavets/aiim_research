"""
exp_062 — GRPO vs pos_discount across all 4 setups (gsm8k / math500 / omnimath /
big-math). 4 columns (datasets) x 2 rows (answer_boxed reward, completion length),
rolling-30. pos_discount is the lead non-entropy candidate; this isolates it vs the
GRPO baseline on every dataset.
"""
import os, re
...
fig.suptitle("exp_062 — GRPO vs pos_discount across all setups (Qwen3-4B-Base, 300 steps)\n"
             "pos_discount = gentle g(t)=tau/(tau+t) discount on the exploration bonus. "
             "top=answer_boxed reward, bottom=completion length.",
             fontsize=11, weight="bold")
out = os.path.join(HERE, "figures", "exp062_grpo_vs_posdiscount.png")
