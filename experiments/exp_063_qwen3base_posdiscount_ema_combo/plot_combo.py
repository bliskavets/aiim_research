"""
exp_063 — pos_discount + EMA-λ combo across all 4 setups. Per dataset overlays the
2 combo setups (pos_discount λ=0.5, λ=0.7) with references: GRPO, pos_discount λ=0.9
(exp_062 default), and FIXED λ=0.7 (exp_062 sweep). 4 cols x 2 rows (boxed/length).
"""
import os, re
...
# (log-suffix, label, color, focal)
CURVES = [
    ("grpo",             "GRPO",                    "#64748b", False),
    ("fixed_lam0.7",     "FIXED λ0.7",              "#2563eb", False),
    ("pos_discount",     "pos_disc λ0.9",           "#f59e0b", False),
    ("posdisc_lam0.5",   "pos_disc λ0.5",           "#16a34a", True),
    ("posdisc_lam0.7",   "pos_disc λ0.7",           "#dc2626", True),
]
...
fig.suptitle("exp_063 — pos_discount + EMA-λ combo (λ=0.5/0.7) vs GRPO / pos_disc λ0.9 / FIXED λ0.7 "
             "(Qwen3-4B-Base, 300 steps)\ntop=answer_boxed reward, bottom=completion length.",
             fontsize=11, weight="bold")
out = os.path.join(HERE, "figures", "exp063_combo.png")
