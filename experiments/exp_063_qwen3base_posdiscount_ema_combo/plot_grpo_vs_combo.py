"""
exp_063 — GRPO vs COMBO (pos_discount + EMA λ=0.5 / λ=0.7) across all 4 setups.
Intermediate-safe: datasets without combo logs yet are simply skipped.
4 columns x 2 rows (answer_boxed reward, completion length), rolling-30.
"""
import os, re
...
CURVES = [
    ("grpo",           "GRPO",          "#64748b"),
    ("posdisc_lam0.5", "COMBO λ=0.5",   "#16a34a"),
    ("posdisc_lam0.7", "COMBO λ=0.7",   "#dc2626"),
]
...
for j, (ds, ds_lab) in enumerate(DATASETS):
    ...
    for suf, lab, c in CURVES:
        ...
        lw = 2.6 if suf != "grpo" else 1.8
        if bx:
            ys = rolling(bx); axb.plot(range(len(ys)), ys, color=c, lw=lw, label=f"{lab} ({lm(bx):+.2f})")
        if cl:
            ys = rolling(cl); axl.plot(range(len(ys)), ys, color=c, lw=lw, label=f"{lab} ({lm(cl):.0f})")
        if bx or cl:
...
fig.suptitle("exp_063 — GRPO vs COMBO (pos_discount + EMA λ=0.5/0.7) across setups (Qwen3-4B-Base, 300 steps) "
             "[intermediate]\ntop=answer_boxed reward, bottom=completion length.",
             fontsize=11, weight="bold")
out = os.path.join(HERE, "figures", "exp063_grpo_vs_combo.png")
