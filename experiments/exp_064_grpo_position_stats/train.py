...
def build_trainer(method, model, tokenizer, args, dataset, reward_funcs, format_tag_patterns=None,
    ...
    if method == "grpo_posstats":
        # plain GRPO + per-position C/logprob logging (loss unchanged)
        from src.grpo_posstats_trainer import GRPOPosStatsTrainer
        return GRPOPosStatsTrainer(**common, top_k=20, conf_micro_bs=_CONF_MICRO_BS,
                                   answer_extractor=_extract_boxed_answer,
                                   stats_path=stats_path or "diag/grpo_posstats.npz",
                                   max_pos=DATA_COMMON["max_completion_tokens"])
    if method == "grpo":
...
def main():
    ...
    stats_path = os.path.join(os.path.dirname(__file__), "diag", f"posstats_{a.dataset}.npz")
