"""
exp_065 — adaptive pos_discount variants vs GRPO + pos_discount λ0.7 (the prior
best) across all 4 setups. 4 cols x 2 rows (boxed / length), rolling-30.
Intermediate-safe (missing/partial logs simply plot what's there).
"""
import os, re
...
CURVES = [
    ...
    ("posdisc_lam0.7",  "pos_disc λ0.7",   "#0ea5e9", False),
    ("adisc_p1",        "adisc_p1",        "#16a34a", True),
    ("adisc_c1",        "adisc_c1",        "#d97706", True),
    ("adisc_pc1",       "adisc_pc1",       "#7c3aed", True),
    ("adisc_pc2",       "adisc_pc2",       "#dc2626", True),
]
...
fig.suptitle("exp_065 — adaptive pos_discount (p1/c1/pc1/pc2) vs GRPO & pos_disc λ0.7 "
             "(Qwen3-4B-Base, 300 steps) [intermediate]\ntop=answer_boxed, bottom=length. "
             "pc1/pc2 (confidence-multiplied bonus) COLLAPSE; p1/c1 stable.",
             fontsize=11, weight="bold")
out = os.path.join(HERE, "figures", "exp065_adaptive_compare.png")
