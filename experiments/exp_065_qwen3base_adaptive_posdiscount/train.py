...
SHAPING_CONFIG = {
    ...
    # exp_065 ADAPTIVE pos_discount variants — base FIXED λ=0.7 (shortlist)
    "adisc_p1":  dict(_SHAPE_BASE, lam=0.7, pos_tau=1024.0, discount_kind="p1", floor=0.3),
    "adisc_c1":  dict(_SHAPE_BASE, lam=0.7, discount_kind="c1", g_min=0.2),
    "adisc_pc1": dict(_SHAPE_BASE, lam=0.7, pos_tau=1024.0, discount_kind="pc1", floor=0.3),
    "adisc_pc2": dict(_SHAPE_BASE, lam=0.7, pos_tau=1024.0, discount_kind="pc2", g_min=0.2, g_max=1.5),
}
GROP_GAMMA1 = 0.75
...
def build_trainer(method, model, tokenizer, args, dataset, reward_funcs, format_tag_patterns=None):
    ...
    if method in ("adisc_p1", "adisc_c1", "adisc_pc1", "adisc_pc2"):
        from src.adaptive_discount_trainer import AdaptiveDiscountTrainer
        return AdaptiveDiscountTrainer(**common, **SHAPING_CONFIG[method],
                                       format_tag_patterns=format_tag_patterns)
...
def main():
    ...
    ap.add_argument("--method", required=True,
                    choices=["grpo", "grpo_grop", "gtpo_ema_flipped_fixed",
                             ...
                             "adisc_p1", "adisc_c1", "adisc_pc1", "adisc_pc2"])
