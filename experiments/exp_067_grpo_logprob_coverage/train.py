...
def build_trainer(method, model, tokenizer, args, dataset, reward_funcs, format_tag_patterns=None,
    ...
    if method == "grpo_lpdump":
        from src.grpo_lpdump_trainer import GRPOLpdumpTrainer
        return GRPOLpdumpTrainer(**common, dump_k=128, conf_micro_bs=_CONF_MICRO_BS,
                                 dump_dir=dump_dir or "diag/lpdump", save_every=1, max_save_steps=100)
    if method == "grpo":
...
def main():
    ...
    ap.add_argument("--method", required=True,
                    choices=["grpo", "grpo_lpdump", "grpo_grop", "gtpo_ema_flipped_fixed",
    ...
    dump_dir = os.path.join(os.path.dirname(__file__), "diag", f"lpdump_{a.dataset}")
    trainer = build_trainer(a.method, model, tokenizer, grpo_args, dataset,
                            REWARD_FUNCS_FULL, format_tag_patterns=format_tag_patterns, dump_dir=dump_dir)
