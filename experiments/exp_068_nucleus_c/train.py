...
def build_trainer(method, model, tokenizer, args, dataset, reward_funcs, format_tag_patterns=None):
    ...
    if method == "nucleus_c":
        from src.nucleus_c_trainer import NucleusCTrainer
        return NucleusCTrainer(**common, **SHAPING_CONFIG[method], format_tag_patterns=format_tag_patterns)
...
def main():
    ...
    ap.add_argument("--min_k", type=int, default=None, help="min nucleus size for nucleus_c")
    a = ap.parse_args()
