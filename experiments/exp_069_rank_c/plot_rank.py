"""
exp_069 — rank-based adaptive-k C (k=clamp(rank_of_sampled_token,1,5)) vs GRPO and
the prior best (pos_discount FIXED λ0.7 top_k=5). Per dataset: boxed (top) + length
(bottom). Intermediate-safe.
"""
import os, re
...
fig.suptitle("exp_069 — rank-C: k=clamp(rank of sampled token, 1, 5) vs GRPO & pos_discount k=5 "
             "(FIXED λ0.7) (Qwen3-4B-Base, 300 steps) [intermediate]\n"
             "k=1 for argmax-sampled (confident) tokens, up to 5 when sampled from the tail; sampling stays 1.0",
             fontsize=11, weight="bold")
out = os.path.join(HERE, "figures", "exp069_rank.png")
