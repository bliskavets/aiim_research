"""
exp_070 — rank-FLOOR C (k=max(rank of sampled token, 5), cap 256) vs GRPO,
the prior best (pos_discount FIXED λ0.7 top_k=5), and exp_069 rank_c (k≤5).
...
fig.suptitle("exp_070 — rank-FLOOR C: k=max(rank of sampled token, 5) (cap 256) vs GRPO, pos_discount k=5 "
             "(FIXED λ0.7), rank_c k≤5 (Qwen3-4B-Base, 300 steps) [intermediate]\n"
             "argmax-sampled tokens (~83%) get k=5 (stable); k grows only on tail-sampled tokens; sampling stays 1.0",
             fontsize=10.5, weight="bold")
out = os.path.join(HERE, "figures", "exp070_rankfloor.png")
