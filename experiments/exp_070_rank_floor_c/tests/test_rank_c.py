...
def test_floor_k5_argmax():
    # exp_070 recipe: k = max(rank, 5) via min_k=5. argmax sampled (rank 1) -> k=5.
    lp = _sorted_lp([0.5, 0.2, 0.15, 0.1, 0.03, 0.02])
    C, k = rank_C(lp, sampled_logprob=lp[..., 0], ranks=torch.tensor([1]), cap=256, min_k=5)
    assert k.item() == 5
    assert torch.allclose(C, -(lp[..., :5].mean()).unsqueeze(0), atol=1e-5)


def test_floor_grows_on_tail():
    # rank 8 with floor 5 -> k=8 (grows past the floor)
    lp = _sorted_lp([0.2, 0.16, 0.14, 0.12, 0.1, 0.09, 0.08, 0.06, 0.05])
    C, k = rank_C(lp, sampled_logprob=lp[..., 7], ranks=torch.tensor([8]), cap=256, min_k=5)
    assert k.item() == 8
...
if __name__ == "__main__":
    ...
    test_min_k_floor(); test_floor_k5_argmax(); test_floor_grows_on_tail(); test_batch_mixed_ranks()
