...
SHAPING_CONFIG = {
    ...
    # exp_071..075 roadmap setups (analysis/exp055-070_deep_analysis.md §5).
    # Base everywhere = current best (FIXED λ0.7, top_k=5, α₁=0.9, α₂=0.1); one change per setup.
    "zvgate":           dict(_SHAPE_BASE, lam=0.7, top_k=5, pos_tau=1024.0),  # 071: + zero-variance gate
    "branch_entropy":   dict(_SHAPE_BASE, lam=0.7, top_k=5, pos_tau=1024.0),  # 072: h=H(head)/log k signal
    "flipped_budget":   dict(_SHAPE_BASE, lam=0.7, top_k=5),                  # 073: budget replaces posdisc
    "surprisal_credit": dict(_SHAPE_BASE, lam=0.7, top_k=5, pos_tau=1024.0),  # 074: additive z(surprisal)
    "final_combo":      dict(_SHAPE_BASE, lam=0.7, top_k=5),                  # 075: gate+branch+budget
}
ROADMAP_METHODS = ("zvgate", "branch_entropy", "flipped_budget", "surprisal_credit", "final_combo")
GROP_GAMMA1 = 0.75
...
def build_trainer(method, model, tokenizer, args, dataset, reward_funcs, format_tag_patterns=None):
    ...
    if method in ROADMAP_METHODS:
        from src.roadmap_trainers import ROADMAP_TRAINERS
        return ROADMAP_TRAINERS[method](**common, **SHAPING_CONFIG[method],
                                        format_tag_patterns=format_tag_patterns)
...
def main():
    ...
    ap.add_argument("--method", required=True,
                    choices=["grpo", "grpo_grop", "gtpo_ema_flipped_fixed",
                             ...
                             "zvgate", "branch_entropy", "flipped_budget", "surprisal_credit", "final_combo"])
