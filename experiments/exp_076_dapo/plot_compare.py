"""
exp_076 — DAPO baseline vs plain GRPO and our best per-token method
(gtpo_ema_flipped FIXED + pos_discount, λ0.7, k5). Per dataset: boxed + length.
"""
import os, re
...
CURVES = [
    ...
    ("posdisc_lam0.7_k5", "our best (posdisc λ0.7 k5)", "#16a34a", False),
    ("dapo",              "DAPO",                    "#dc2626", True),
]
...
fig.suptitle("exp_076 — DAPO (Clip-Higher 0.2/0.28 + token-level loss + overlong masking) "
             "vs GRPO vs our best per-token method (Qwen3-4B-Base, 300 steps)",
             fontsize=11, weight="bold")
out = os.path.join(HERE, "figures", "exp076_dapo.png")
