"""
exp_077 — our best per-token shaping ON TOP of DAPO vs: DAPO baseline (exp_076),
...
CURVES = [
    ...
    ("dapo",              "DAPO baseline",              "#2563eb", False),
    ("dapo_shaped",       "shaping on DAPO",            "#dc2626", True),
]
...
fig.suptitle("exp_077 — our per-token shaping (posdisc λ0.7 k5) ON TOP of DAPO vs DAPO baseline, "
             "shaping-on-GRPO, and GRPO (Qwen3-4B-Base, 300 steps)\n"
             "does the per-token add-on still help under a stronger RL algorithm?",
             fontsize=11, weight="bold")
out = os.path.join(HERE, "figures", "exp077_dapo_shaped.png")
