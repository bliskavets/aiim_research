...
SHAPING_CONFIG = {
    ...
    # exp_078: our shaping (pos_discount FIXED, λ=0.7, k=5) applied ON TOP of GSPO.
    "gspo_shaped": dict(_SHAPE_BASE, lam=0.7, top_k=5, pos_tau=1024.0),
}
# exp_078: GSPO (arXiv:2507.18071) — sequence-level importance sampling with the paper's
# clip range (3e-4 / 4e-4). loss_type='grpo' reproduces the per-sequence-mean aggregation
# of the GSPO objective (with a sequence-level ratio the per-token weights are constant).
GSPO_METHODS = ("gspo", "gspo_shaped")
GSPO_CONFIG = {"importance_sampling_level": "sequence", "epsilon": 3e-4,
               "epsilon_high": 4e-4, "loss_type": "grpo"}
GROP_GAMMA1 = 0.75
...
def build_trainer(method, model, tokenizer, args, dataset, reward_funcs, format_tag_patterns=None):
    ...
    if method == "gspo":
        return GRPOTrainer(**common)                          # plain GRPOTrainer + GSPO config knobs
    if method == "gspo_shaped":
        from src.novel_trainers import PosDiscountTrainer     # our shaping ON TOP of GSPO
        return PosDiscountTrainer(**common, **SHAPING_CONFIG["gspo_shaped"],
                                  format_tag_patterns=format_tag_patterns)
...
def main():
    ...
    if a.method in GSPO_METHODS:
        for k, v in GSPO_CONFIG.items():
            setattr(grpo_args, k, v)
        print(f"[GSPO] {GSPO_CONFIG} (sequence-level IS, paper clip range)")
