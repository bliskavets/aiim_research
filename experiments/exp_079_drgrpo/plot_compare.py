"""
exp079 — our per-token shaping ON TOP of Dr.GRPO vs: Dr.GRPO baseline,
our best shaping on plain GRPO (posdisc λ0.7 k5), and plain GRPO.
Tests whether the per-token add-on still helps under a stronger RL algorithm.
"""
import os, re
...
CURVES = [
    ...
    ("posdisc_lam0.7_k5", "shaping on GRPO (posdisc)",  "#16a34a", False),
    ("drgrpo",              "Dr.GRPO baseline",              "#2563eb", False),
    ("drgrpo_shaped",       "shaping on Dr.GRPO",            "#dc2626", True),
]
...
fig.suptitle("exp079 — our per-token shaping (posdisc λ0.7 k5) ON TOP of Dr.GRPO (unbiased loss+rewards, arXiv:2503.20783) vs baseline, "
             "shaping-on-GRPO, and GRPO (Qwen3-4B-Base, 300 steps)\n"
             "does the per-token add-on still help under a stronger RL algorithm?",
             fontsize=11, weight="bold")
out = os.path.join(HERE, "figures", "exp079_drgrpo_shaped.png")
os.makedirs(os.path.dirname(out), exist_ok=True)
fig.tight_layout(rect=[0, 0, 1, 0.93]); fig.savefig(out, dpi=140)
print(f"saved {out}")
print(f"\n{'dataset':10s} {'method':28s} {'L50_len':>9s} {'L50_boxed':>10s} {'steps':>6s}")
for ds, m, ml, mb, n in rows:
    print(f"{ds:10s} {m:28s} {ml:>9.0f} {mb:>+10.2f} {n:>6d}")
