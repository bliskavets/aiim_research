...
for j, (ds, ds_lab) in enumerate(DATASETS):
    ...
    for suf, lab, c, focal in CURVES:
        ...
        bx = col(p, "reward_answer_exact/mean"); cl = col(p, "completions/mean_length")
    ...
    axb.set_title(f"{ds_lab} — answer_exact", fontsize=11, weight="bold")
    axb.axhline(0, color="#cbd5e1", lw=0.6, ls="--"); axb.grid(alpha=0.3); axb.legend(fontsize=8, loc="lower right")
    axb.set_xlabel("step"); axb.set_ylabel("answer_exact reward")
