...
MODEL_CONFIG = {
    ...
    "max_seq_length": 2560,          # exp_050 budget: 512 prompt + 2048 completion
    "lora_rank": 64,
    "load_in_4bit": False,
    "fast_inference": True,
...
TRAINING_CONFIG = {
    ...
    "num_generations": 4,
    "max_steps": int(os.environ.get("SMOKE_MAX_STEPS", 500)),   # exp_050: Llama takes off at ~250-350
    "save_steps": 9999,
    "max_grad_norm": 1.0,
    "report_to": "none",
    "seed": SEED,
...
DATA_COMMON = {"max_prompt_tokens": 512, "max_completion_tokens": 2048,
...
SHAPING_CONFIG = {
    ...
    # exp_077: our best shaping (pos_discount FIXED, λ=0.7, k=5) applied ON TOP of DAPO.
    "dapo_shaped": dict(_SHAPE_BASE, lam=0.7, top_k=5, pos_tau=1024.0),
    # exp_080: "Ours" = our best per-token shaping on plain GRPO (pos_discount FIXED λ0.7 k5).
    "ours": dict(_SHAPE_BASE, lam=0.7, top_k=5, pos_tau=1024.0),
}
# exp_076/077: DAPO config knobs (Clip-Higher + token-level loss + overlong masking).
# TRL 0.23.1 already defaults loss_type='dapo' (token-level); DAPO adds asymmetric
# clipping and truncated-completion masking. (Dynamic re-sampling is not native to TRL
# 0.23.1 — documented in the README; the zero-variance skip (exp_071) is our analogue.)
DAPO_METHODS = ("dapo", "dapo_shaped")
DAPO_CONFIG = {"epsilon": 0.2, "epsilon_high": 0.28, "loss_type": "dapo",
               "mask_truncated_completions": True}
GROP_GAMMA1 = 0.75
PRINT_EVERY_STEPS = 10
# ── exp_050 harness: custom working-out/SOLUTION tags (Llama-friendly format) ──
...
# ── rewards (exp_050 harness: graded format + graded answer, exp_026 family) ──
...
def build_trainer(method, model, tokenizer, args, dataset, reward_funcs, format_tag_patterns=None):
    ...
    if method == "gtpo_ema_flipped":
        # ORIGINAL exp_050 trainer (overrides _compute_loss — the pre-FIX path that unsloth's
        # compiled GRPOTrainer bypasses / B=1-degenerates; see shaping_bypassed_by_unsloth memo)
        from src.gtpo_ema_flipped_trainer import GTPOEMAFlippedTrainer
        return GTPOEMAFlippedTrainer(**common, **SHAPING_CONFIG["gtpo_ema_flipped_fixed"],
    ...
    if method == "ours":
        from src.novel_trainers import PosDiscountTrainer     # best per-token shaping on plain GRPO
        return PosDiscountTrainer(**common, **SHAPING_CONFIG["ours"],
                                  format_tag_patterns=format_tag_patterns)
    if method == "dapo":
        return GRPOTrainer(**common)                          # plain GRPOTrainer + DAPO config knobs
    if method == "dapo_shaped":
        from src.novel_trainers import PosDiscountTrainer     # our best shaping ON TOP of DAPO
        return PosDiscountTrainer(**common, **SHAPING_CONFIG["dapo_shaped"],
                                  format_tag_patterns=format_tag_patterns)
...
def main():
    ...
    ap.add_argument("--method", required=True,
                    choices=["grpo", "grpo_grop", "gtpo_ema_flipped", "gtpo_ema_flipped_fixed",
                             "sign_gate", "pos_discount", "raw_c", "ref_delta", "nucleus_c",
                             "dapo", "dapo_shaped", "ours"])
    ...
    print(f"=== exp_082 [{a.dataset} | {a.method}] Llama-3.2-3B-Instruct (exp_050 harness) — steps={TRAINING_CONFIG['max_steps']} "
    ...
    if getattr(tokenizer, "chat_template", None) is None:
        ...
        tmpl = None
        for src in (MODEL_CONFIG["model_name"], "unsloth/Llama-3.2-3B-Instruct"):
            try:
    ...
    if a.method in DAPO_METHODS:
        for k, v in DAPO_CONFIG.items():
            setattr(grpo_args, k, v)
        print(f"[DAPO] {DAPO_CONFIG} (clip-higher + token-level loss + overlong masking)")

    from src.format_tag_mask import encode_tag_patterns
    format_tag_patterns = encode_tag_patterns(tokenizer, [REASONING_START, REASONING_END, SOLUTION_START, SOLUTION_END])
