...
BoN ($N{=}7$)              & 0.70 & \textcolor{orange!85!black}{0.73} \\
\method{} (2 ep., $N{=}7$) & \textbf{0.77} & \textbf{0.80} \\
\bottomrule
\end{tabular}
\end{sc}
...
We also evaluate \method{} at two scales beyond the 8B models used above: Qwen3-1.7B (smaller) and Qwen3-32B-FP8 (larger), on a held-out MATH-500 subset (seed=42). Results including wall-clock times are shown in \Cref{tab:small-large-models} in the appendix. \method{} consistently outperforms Best-of-\(N\) at a fixed candidate count and at matched wall-clock time across all three scales. For Qwen3-1.7B, \method{} with two optimization epochs reaches 0.88, surpassing the best BoN baseline (Skywork-Reward-V2-Llama-3.1-8B, $N=70$: 0.86) while using only the 1.7B generator (roughly $5.7\times$ less VRAM than BoN paired with an 8B reward model). At 32B (\cref{tab:qwen32b} in the appendix), \method{} reaches 0.97 from a strong 0.80 greedy baseline, again ahead of Best-of-\(N\) reranking with an external reward model.
...
\textcolor{orange!85!black}{The model-scale results below use a held-out MATH-500 subset that is deliberately harder than the full benchmark (the 8B greedy baseline scores 0.61 on it).}
...
\end{sc}
\end{small}
\caption{MATH-500 subset results at 1.7B and 8B scales. Wall-clock measured on a single A100\,80\,GB GPU; \texttt{--} indicates not measured. \textcolor{orange!85!black}{Evaluated on a held-out subset that is deliberately harder than the full benchmark (the 8B greedy baseline scores 0.61 here).}}
...
\end{sc}
\end{small}
\caption{\textcolor{orange!85!black}{Held-out hard} MATH-500 subset, Qwen3-32B-FP8. \method{} beats a strong greedy baseline and external-RM Best-of-\(N\) without using a reward model.}
\label{tab:qwen32b}
\end{table}
