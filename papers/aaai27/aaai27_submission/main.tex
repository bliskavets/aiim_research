...
We also evaluate \method{} at two scales beyond the 8B models used above: Qwen3-1.7B (smaller) and Qwen3-32B-FP8 (larger), on a held-out MATH-500 subset (seed=42). Results including wall-clock times are shown in \Cref{tab:small-large-models} in the appendix. \method{} consistently outperforms Best-of-\(N\) at a fixed candidate count and at matched wall-clock time across all three scales. For Qwen3-1.7B, \method{} with two optimization epochs reaches 88.0, surpassing the best BoN baseline (Skywork-Reward-V2-Llama-3.1-8B, $N=70$: 86.0) while using only the 1.7B generator (roughly $5.7\times$ less VRAM than BoN paired with an 8B reward model). At 32B (\cref{tab:qwen32b} in the appendix), \method{} reaches 97.0 from a strong 80.0 greedy baseline, again ahead of Best-of-\(N\) reranking with an external reward model.
...
\end{sc}
\end{small}
\caption{MATH-500 results (\%) at 1.7B and 8B scales, \textcolor{orange!85!black}{evaluated on a held-out subset}. \(N\) is the number of sampled candidates. Wall-clock measured on a single A100\,80\,GB GPU; \texttt{--} indicates not measured.}
...
\end{sc}
\end{small}
\caption{\textcolor{orange!85!black}{Held-out} MATH-500 subset, Qwen3-32B-FP8; accuracy in \% and \(N\) is the number of sampled candidates. \method{} beats a strong greedy baseline and external-RM Best-of-\(N\) without using a reward model.}
\label{tab:qwen32b}
\end{table}
