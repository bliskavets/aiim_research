...
\textcolor{orange!85!black}{The procedure is not sensitive to the exact group-size setting: sweeping the minimum group size \(m_{\min}\) over \(\{1,2,4,8\}\) on MATH-500 changes accuracy by less than 2 points, with very large groups limited only by the size of the candidate pool. Group construction itself, however, matters: on a 50-problem MATH-500 subset, pure best-versus-worst feedback (both groups capped at one) yields 82.0, size-2 groups 86.0, and the default 4/3 configuration 92.0; the gain from richer groups is monotone.}
