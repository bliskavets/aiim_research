...
We also conduct an additional analysis of how the prompt's instructions to produce a more confident estimates affect the model's judgement accuracy. The ablation details can be found in Appendix~\ref{sec:app-prompt-calibration}.
...
To assess generalization beyond math and open-ended preference tasks, we evaluate \method{} on IFEval \citep{zhou2023instruction}, a strict instruction-following benchmark, and on a subset of MMLU-Pro STEM \citep{wang2024mmlu} covering multi-step reasoning in science and engineering. Results with Qwen3-8B-FP8 are shown in \Cref{tab:additional-benchmarks}. Compared to only 4-point gains from Best-of-\(N\), \method{} improves IFEval prompt accuracy by 28 points and MMLU-Pro STEM accuracy by 20 points, indicating that iterative self-refinement generalizes beyond mathematical problem-solving to instruction-following and broader STEM reasoning.

\paragraph{Discussion.} Taken together, the experiments draw a consistent picture. \method{} helps most where the model can verify its own outputs: the largest improvements appear on MATH-500, AIME, and MMLU-Pro STEM, where the judge's correctness signal is comparatively reliable, whereas the margins on XSTest are narrower because the base models already score high and the ceiling is close. Preference-oriented tasks benefit as well, but there the decisive factor is the multi-aspect judging prompt, which penalizes truncation and repetition explicitly rather than relying on correctness alone. The comparison with SPO further suggests that iterating on responses is more robust than iterating on the prompt: tuned prompts transfer poorly when calibration examples come from a different domain, while the textual gradient is recomputed for every input.
}
...
\method{}, 2 ep., $N{=}7$    & \textbf{70.0\%} & \textbf{75.7\%} & \textbf{68.0\%} \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
\caption{IFEval and MMLU-Pro STEM results (Qwen3-8B-FP8, seed=42): prompt- and instruction-level accuracy for IFEval, overall accuracy for MMLU-Pro STEM. BoN uses FsfairX-LLaMA3-RM-v0.1.}
...
\method{} aims to improve the helpfulness and correctness of LLM outputs at inference time, and our XSTest results indicate that these gains do not come at the expense of safety behavior. The method nevertheless inherits the values encoded in the underlying model: the preference signal is elicited from the model itself, so systematic blind spots in its self-assessment are reinforced rather than corrected, and it is no substitute for safety training or external oversight in high-stakes settings.
}
