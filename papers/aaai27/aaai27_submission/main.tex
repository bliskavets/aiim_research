...
For math-oriented tasks, we anneal the generation temperature from 0.8--1.0 in the initial stage to 0.7 in subsequent stages. Unless otherwise noted, we sample \(N=7\) hypotheses per generation stage and run two optimization epochs per benchmark, excluding the initial generation stage. Because the good and bad sets are selected automatically, we do not fix their sizes; the median size of each group is two to three hypotheses. \textcolor{orange!85!black}{Unless stated otherwise, results are single runs with a fixed seed; selected analyses are repeated over three seeds.} \textcolor{green!50!black}{Regarding cost, the overhead of two optimization epochs with \(N=7\) is dominated by the extra hypothesis generations rather than by judging, which reads only a few forced label tokens and reuses cached margins.} \textcolor{orange!85!black}{Concrete per-problem wall-clock on a single A100\,80\,GB GPU is reported in \cref{tab:small-large-models}.}
...
Llama-3.1-8B-Instruct & 60.2 & \textcolor{orange!85!black}{29.0} & \textcolor{orange!85!black}{27.0} & 95.1 \\
\addlinespace
...
Llama-3.1-8B-Instruct + Best-of-\(N\) & 57.4 & \textcolor{orange!85!black}{26.5} & \textcolor{orange!85!black}{25.0} & 94.4 \\
Llama-3.1-8B-Instruct + TPO & 58.8 & \textcolor{orange!85!black}{27.5} & \textcolor{orange!85!black}{24.0} & 93.3 \\
...
\method{} + RM (Llama-3.1-8B-Instruct) & 59.4 & \textcolor{orange!85!black}{28.0} & \textcolor{orange!85!black}{26.0} & 95.1 \\
\method{} (Qwen3-8B) & 92.0 & 74.9 & \textcolor{orange!85!black}{55.0} & 93.5 \\
\method{} (Llama-3.1-8B-Instruct) & 60.2 & \textcolor{orange!85!black}{29.0} & \textcolor{orange!85!black}{27.0} & 95.1 \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
...
\section{\textcolor{orange!85!black}{Reasoning mode and model-scale results}}
\label{app:scaling-tables}
\textcolor{orange!85!black}{Unlike \cref{tab:results-by-model}, which reports full-benchmark scores, the results in this appendix are computed on small evaluation sets: AIME~2026 (30 problems, \cref{tab:aime2026}) and a held-out hard MATH-500 subset (\cref{tab:small-large-models,tab:qwen32b}). Absolute values are therefore not comparable to the main results table---for instance, the 8B greedy baseline scores only 0.61 on the held-out subset versus 84.4 on full MATH-500---and only comparisons within each individual table are meaningful.}
...
\end{tabular}
\end{sc}
\caption{AIME~2026 accuracy in reasoning (thinking) mode (Qwen3.5-4B is a hybrid-reasoning model run in thinking mode); BoN uses the Skywork-Reward-V2-Llama-3.1-8B reward model, whereas \method{} requires no external reward model. \textcolor{orange!85!black}{Scores are on the 30-problem AIME~2026 set and are not comparable to the full-benchmark results in \cref{tab:results-by-model}.}}
\label{tab:aime2026}
\end{table}
\begin{table*}[t]
\centering
...
\end{sc}
\end{small}
\caption{MATH-500 subset results at 1.7B and 8B scales. Wall-clock measured on a single A100\,80\,GB GPU; \texttt{--} indicates not measured. \textcolor{orange!85!black}{The held-out hard subset is substantially harder than the full benchmark (the 8B greedy baseline scores 0.61 on it versus 84.4 on the full set), so absolute values are not comparable to \cref{tab:results-by-model}.}}
