...
\begin{figure*}[t]
\centering
\includegraphics[width=0.68\textwidth]{figures/figure2_new_28012026.pdf}
\caption{
...
We form groups instead of comparing only extremes candidates. Contrasting a set of good responses \(\mathcal{G}\) with a set of bad responses \(\mathcal{B}\) yields a more stable and informative signal. By leveraging group-wise comparisons, we achieve more stable guidance and improved downstream transfer.
...
\textcolor{green!50!black}{The procedure is likewise robust to its two main design knobs. Swapping the default aspect set for a single generic quality rubric or a task-specific variant moves MATH-500 accuracy by at most 2.8 points, within the noise band at \(N{=}500\), and sweeping the minimum group size \(m_{\min}\) over \(\{1,2,4,8\}\) changes it by less than 2 points, with very large groups limited only by the size of the candidate pool.}

\textcolor{green!50!black}{Because SPO relies on GPT-4.1 as its prompt optimizer, we also re-ran it with the base Qwen3-8B optimizing its own prompt: the search never improves on the seed prompt within six rounds, as the model's noisy self-evaluations are too weak a comparison signal for prompt selection. Prompt-level self-optimization hinges on a stronger external model, whereas the token-level margin \method{} extracts from the same base model remains informative.}
...
\section*{Limitations}
\noindent \method{} relies on the base LLM's own judgments for scoring and gradient extraction, so miscalibration or systematic biases in its self-assessment can propagate through the refinement loop. The method also incurs linear inference-time overhead with the number of sampled hypotheses and optimization epochs, prohibitive in latency-critical settings. \textcolor{green!50!black}{Reasoning-mode decoding reaches a higher raw ceiling on math, but its generation length is difficult to control and, under a fixed token budget, a nontrivial fraction of traces does not terminate; \method{} targets the complementary operating point of predictable budgets, leaving the combination to future work.} Finally, judge aspects and weights are hyperparameters, though results here are robust to their formulation.
