...
\begin{abstract}
We present \method{}, a test-time alignment method for refining large language model (LLM) outputs at inference time without retraining or relying on external reward models. \method{} uses the same LLM as both generator and self-judge: candidate responses are scored using a contrastive label judgment prompt and the resulting log-probability margin serves as a continuous preference signal. An iterative refinement algorithm then uses these scores to form \textcolor{orange!85!black}{contrastive candidate sets}, extract a textual gradient, and guide subsequent hypothesis generations under a token budget.
Across \textcolor{orange!85!black}{math reasoning (MATH-500, AIME~2026), instruction following (IFEval), broad STEM reasoning (MMLU-Pro), open-ended preference (AlpacaEval), and safety (XSTest)}, and \textcolor{orange!85!black}{across model families and scales (Qwen3 from 1.7B to 32B and Llama-3.1-8B)}, \method{} consistently improves accuracy and preference metrics relative to existing test-time alignment approaches, with ablations showing that these gains are robust under fixed inference budgets and do not rely on external reward models. The approach is resource-lean and plug-and-play, and we release the code and prompts to facilitate adoption.
\end{abstract}
...
\begin{itemize}
    ...
    \item Finally, we present an iterative test-time alignment procedure that integrates self-judged scoring, grouped selection, and textual gradients to progressively improve generations. \textcolor{orange!85!black}{We release the code, prompts, scoring configurations, and the accompanying artifacts of our method to support reproducibility.}
\end{itemize}
...
For math-oriented tasks, we anneal the generation temperature from 0.8--1.0 in the initial stage to 0.7 in subsequent stages. Unless otherwise noted, we sample \(N=7\) hypotheses per generation stage and run two optimization epochs per benchmark, excluding the initial generation stage. Because the good and bad sets are selected automatically, we do not fix their sizes; the median size of each group is two to three hypotheses. \textcolor{orange!85!black}{Unless stated otherwise, results are single runs with a fixed seed; selected analyses are repeated over three seeds.} \textcolor{green!50!black}{Regarding cost, the overhead of two optimization epochs with \(N=7\) is dominated by the extra hypothesis generations rather than by judging, which reads only a few forced label tokens and reuses cached margins.} \textcolor{orange!85!black}{Concrete per-problem wall-clock on a single A100\,80\,GB GPU is reported in \cref{tab:small-large-models} in the appendix.}
...
Qwen3-8B & 92.0 & 74.9 & \textcolor{orange!85!black}{54.8} & 93.5 \\
Llama-3.1-8B-Instruct & 60.2 & \textcolor{orange!85!black}{29.3} & \textcolor{orange!85!black}{26.8} & 95.1 \\
\addlinespace
...
Llama-3.1-8B-Instruct (Base) & \textcolor{orange!85!black}{51.9} & \textcolor{orange!85!black}{22.6} & \textcolor{orange!85!black}{22.9} & 90.6 \\
Llama-3.1-8B-Instruct + Best-of-\(N\) & 57.4 & \textcolor{orange!85!black}{26.5} & \textcolor{orange!85!black}{24.7} & 94.4 \\
Llama-3.1-8B-Instruct + TPO & 58.8 & \textcolor{orange!85!black}{27.5} & \textcolor{orange!85!black}{23.6} & 93.3 \\
...
\method{} + RM (Llama-3.1-8B-Instruct) & 59.4 & \textcolor{orange!85!black}{28.2} & \textcolor{orange!85!black}{25.7} & 95.1 \\
\method{} (Qwen3-8B) & 92.0 & 74.9 & \textcolor{orange!85!black}{54.8} & 93.5 \\
\method{} (Llama-3.1-8B-Instruct) & 60.2 & \textcolor{orange!85!black}{29.3} & \textcolor{orange!85!black}{26.8} & 95.1 \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
\caption{Ablation: \method{} with an external reward model (judging) vs. internal judge. Metrics as in \cref{tab:results-by-model}. \textcolor{orange!85!black}{On Llama the two variants perform comparably, indicating the self-judge is a strong, lightweight alternative to an external reward model.}}
...
\textcolor{orange!85!black}{We also ablate how a fixed inference budget is split between the number of samples per stage \(N\) and the number of optimization epochs \(T\) (\cref{tab:ablation-N-epochs}): accuracy stays within a narrow band across allocations, and the default setting performs best.} AlpacaEval reward dynamics across epochs are in Appendix~\ref{app:ablation-more}. Temperature ablations (\cref{fig:temp-ablation} in the Appendix) confirm that a lower judging temperature improves ranking quality while a higher generation temperature (\(\approx 0.9\)) maximizes solution discovery.
...
\textcolor{orange!85!black}{Group construction matters: enlarging the contrastive groups from pure best-versus-worst feedback to the default configuration raises MATH-500 accuracy monotonically (\cref{fig:group-size} in the appendix), confirming that guidance distilled from richer good and bad sets is more effective than extreme-only comparisons.}

{\color{blue}
\subsection{Reasoning mode and model scale}
\label{sec:scaling-exp}
To verify that \method{} remains effective for reasoning (thinking) models, we evaluate on AIME~2026 \citep{dekoninck2026beyond}, a competition-level mathematics benchmark well suited for inference-time compute scaling. Both models run in reasoning mode\textcolor{orange!85!black}{ (for the hybrid-reasoning Qwen3.5-4B checkpoint we enforce thinking via the chat template)} with a maximum sequence length of 32,768 tokens. Results are reported in \cref{tab:aime2026}. On Qwen3-8B, \method{} outperforms Best-of-\(N\) with Skywork-Reward-V2-Llama-3.1-8B \citep{liu2025skywork} while using no external reward model. On Qwen3.5-4B (thinking), \method{} further improves over the greedy baseline by seven points, \textcolor{orange!85!black}{whereas Best-of-\(N\) with the same external reward model only matches greedy decoding: the reward model recovers the best answer available among the sampled candidates, but sampling at \(N=7\) yields no headroom that greedy decoding does not already reach.}
...
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{Qwen3-8B} & \textbf{Qwen3.5-4B} \\
\midrule
Baseline (greedy)          & 56.7 & 73.3 \\
BoN ($N{=}7$)              & 70.0 & \textcolor{orange!85!black}{73.3} \\
\method{} (2 ep., $N{=}7$) & \textbf{76.7} & \textbf{80.0} \\
\bottomrule
\end{tabular}
\end{sc}
\caption{AIME~2026 accuracy (\%) in reasoning (thinking) mode (Qwen3.5-4B is a hybrid-reasoning model run in thinking mode); BoN uses the Skywork-Reward-V2-Llama-3.1-8B reward model, whereas \method{} requires no external reward model.}
\label{tab:aime2026}
\end{table}

We also evaluate \method{} at two scales beyond the 8B models used above: Qwen3-1.7B (smaller) and Qwen3-32B-FP8 (larger), on a held-out MATH-500 subset (seed=42). Results including wall-clock times are shown in \Cref{tab:small-large-models} in the appendix. \method{} consistently outperforms Best-of-\(N\) at a fixed candidate count and at matched wall-clock time across all three scales. For Qwen3-1.7B, \method{} with two optimization epochs reaches 88.0, surpassing the best BoN baseline (Skywork-Reward-V2-Llama-3.1-8B, $N=70$: 86.0) while using only the 1.7B generator (roughly $5.7\times$ less VRAM than BoN paired with an 8B reward model). At 32B (\cref{tab:small-large-models} in the appendix), \method{} reaches 97.0 from a strong 80.0 greedy baseline, again ahead of Best-of-\(N\) reranking with an external reward model.

\subsection{Instruction following and broader STEM reasoning}
\label{sec:broader-benchmarks}
To assess generalization beyond math and open-ended preference tasks, we evaluate \method{} on IFEval \citep{zhou2023instruction}, a strict instruction-following benchmark, and on a subset of MMLU-Pro STEM \citep{wang2024mmlu} covering multi-step reasoning in science and engineering. Results with Qwen3-8B-FP8 are reported in the appendix (\cref{tab:additional-benchmarks}). \textcolor{orange!85!black}{On MMLU-Pro STEM, \method{} improves accuracy from 71.2 to 77.3 (three-seed mean, McNemar $p<10^{-3}$), while TPO drops below the greedy baseline (62.6): its general-purpose reward model favors helpful-sounding prose over correct multiple-choice reasoning. On IFEval \method{} gains 2.5 points and matches Best-of-\(N\); deterministic scoring rules out verbosity inflation. Together the benchmarks show self-judged refinement helps most where correctness is verifiable and external preference signals are weakest.}
...
\section*{Limitations}
\noindent \textcolor{orange!85!black}{\method{} is a test-time alignment method and shares that family's profile: it improves a frozen base model without retraining or an external reward model, in exchange for inference-time compute that grows with the number of hypotheses and epochs, with the judge aspects and weights as lightweight per-task choices. It is thus most attractive under a fixed inference-time budget and is complementary to training-time alignment.}
...
\noindent We analyze how the reward model score evolves across optimization epochs on AlpacaEval (\cref{fig:ablation-rewardscore}); all experiments use Qwen3-8B. \textcolor{orange!85!black}{We also report per-benchmark results on IFEval and MMLU-Pro STEM here (\cref{tab:additional-benchmarks}).}
...
\end{sc}
\end{small}
\caption{\textcolor{orange!85!black}{IFEval (all 541 prompts) and MMLU-Pro STEM (500 questions) with Qwen3-8B-FP8: prompt- and instruction-level strict accuracy for IFEval, accuracy for MMLU-Pro STEM. \method{} MMLU-Pro is the mean of three seeds ($\pm$1.3). Absolute IFEval scores depend on the prompt template; all methods share the same zero-shot configuration. On IFEval all refinement/selection methods land in the same band; \method{}'s clear edge is on verifiable reasoning (MMLU-Pro STEM). \texttt{--} = not measured.}}
\label{tab:additional-benchmarks}
\end{table}

\textcolor{orange!85!black}{\paragraph{Aspect choice.} Judge aspects are task-specific hyperparameters. On the 50-problem development MATH-500 subset, a single task-relevant \emph{verification} aspect performs best: substituting a generic overall-quality aspect, or diluting verification with generic usefulness and completeness aspects, lowers accuracy from 92 to 84 in both cases (greedy baseline: 82). We therefore use verification alone on verifiable tasks and reserve multi-aspect weighting for open-ended ones (AlpacaEval, XSTest).}
...
\section{\textcolor{orange!85!black}{Model-scale results}}
\label{app:scaling-tables}
\textcolor{orange!85!black}{The model-scale results below are evaluated on a held-out MATH-500 subset; scores are percentages.}
...
Baseline          & --                                    &   1 & 56.0          & 22.0  \\
BoN               & FsfairX-LLaMA3-RM-v0.1               &  21 & 62.0          & 43.0  \\
BoN               & FsfairX-LLaMA3-RM-v0.1               &  70 & 66.0          & 163.7 \\
BoN               & Skywork-Reward-V2-Qwen3-8B            &  21 & 68.0          & 48.5  \\
BoN               & Skywork-Reward-V2-Qwen3-8B            &  70 & 72.0          & 168.7 \\
BoN               & Skywork-Reward-V2-Llama-3.1-8B        &  70 & 86.0          & 144.1 \\
\method{} (1 ep.) & --                                    &  14 & 84.0          & 105.5 \\
\method{} (2 ep.) & --                                    &  21 & \textbf{88.0} & 152.4 \\
\midrule
\multicolumn{5}{l}{\textit{Qwen3-8B}} \\
Baseline          & --                                    &   1 & 61.0          & 37.0  \\
BoN               & FsfairX-LLaMA3-RM-v0.1               & 120 & 72.0          & 287.7 \\
BoN               & Skywork-Reward-V2-Qwen3-8B            & 120 & 76.0          & 270.7 \\
\method{} (1 ep.) & --                                    &  14 & 93.0          & 132.9 \\
\method{} (2 ep.) & --                                    &  21 & \textbf{97.0} & 245.6 \\
\midrule
\multicolumn{5}{l}{\textit{Qwen3-32B-FP8}} \\
Baseline          & --                                    &   1 & 80.0          & --    \\
BoN               & Skywork-Reward-V2-Llama-3.1-8B        &   7 & 94.0          & --    \\
\method{} (2 ep.) & --                                    &  21 & \textbf{97.0} & --    \\
\bottomrule
\end{tabular*}
\end{sc}
\end{small}
\caption{MATH-500 results (\%) at 1.7B, 8B, and 32B scales, \textcolor{orange!85!black}{evaluated on a held-out subset}. \(N\) is the number of sampled candidates. Wall-clock measured on a single A100\,80\,GB GPU; \texttt{--} indicates not measured (Qwen3-32B-FP8 wall-clock was not recorded).}
