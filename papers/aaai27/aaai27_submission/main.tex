...
\method{} consistently improves performance across these benchmarks without relying on an external reward model. On Qwen3-8B, \method{} improves accuracy and preference metrics across all evaluated domains. AlpacaEval win rate \textcolor{orange!85!black}{(against the AlpacaEval reference model, GPT-4-class judge)} increases from 57.8 to 74.9, XSTest accuracy from 89.3 to 93.5, and MATH-500 accuracy from 84.4 to 92.0. \method{} also outperforms inference-time baselines that rely on explicit reward models (e.g., Best-of-\(N\)+RM, TPO) and generic LLM-as-a-judge approaches, under comparable budgets. The same procedure also applies to Llama-3.1-8B-Instruct, suggesting that \method{}’s self-judged textual-gradient refinement generalizes across model families.
...
\noindent We report accuracy on MATH-500 and XSTest, and both pairwise win rate and length-controlled win rate on AlpacaEval.\footnote{\textcolor{orange!85!black}{AlpacaEval win rates use a fixed in-house protocol: a single judge configuration over all 805 instructions, comparing against a fixed reference model that is weaker than the live leaderboard's GPT-4-turbo reference. Absolute win rates therefore sit higher than the current leaderboard and are not comparable across papers; only the relative ordering within each table is meaningful.}} \Cref{tab:results-by-model} summarizes results for Qwen3-8B and Llama-3.1-8B-Instruct under \method{} alongside competitive baselines including TPO \citep{litest}, SPO \citep{xiang2025self}, Best-of-\(N\), and the base models.

For Qwen3-8B, \method{} performs best on both math and general tasks. On MATH-500, it reaches the accuracy of \(92.0\), outperforming the base model as well as both Best-of-\(N\) and TPO. On AlpacaEval, it attains a win rate of \(74.9\) and LC win rate of \(63.0\), exceeding all baselines.  This advantage on math tasks appears to be driven by self-judging. Qwen3-8B evaluates the correctness of its own generations more reliably than an external reward model, which leads to steadier improvements and higher final scores (\cref{fig:judge-rm-dynamics,tab:rm-ablation}). \textcolor{green!50!black}{To rule out length effects, we ran a randomized head-to-head comparison of \method{} and base responses with a GPT-4.1 judge (200 prompts, response order randomized): \method{} is preferred in 57.5\% of pairs while its responses are marginally shorter at the median, so the gains reflect content rather than verbosity.}
...
\method{} (Qwen3-8B) & 92.0 & 74.9 & 63.0 & 93.5 \\
