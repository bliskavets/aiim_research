...
<!-- We ran the requested studies during the rebuttal -->
<!-- We conducted the requested analyses during the rebuttal period and addressed each concern below, with experimental numbers in every case. -->
...
<!-- On v1 the automatic scoring agrees with the human judgement 85.1% of the time (κ = 0.67), indicating that the automatic scoring broadly aligns with the judgments of a domain-knowledgeable evaluator. -->
<!-- so the scoring reflects how a knowledgeable person grades these answers rather than drifting from it.  -->
<!-- On v2 no LLM is involved in scoring at all: answers are numeric and are checked against the output of an executable reference plan, and 178 of the 200 sampled environments reproduce their gold answer under execution, an 89% validity rate. -->

<!-- This also answers why v1 uses a judge in the first place.  -->
...
<!-- There is no single string a deterministic rule could match here, which is exactly why v1 needs a semantic judge and why v2, whose answers are numeric by construction, does not.  -->
...
<!-- The human study confirms that the judge closely tracks a knowledgeable reader's grading at 85.1%, so it is a calibrated instrument rather than an added source of uncertainty.  -->
During development, we also involved a human evaluator while designing and validating the generation pipeline. 
<!-- During development we also kept a human in the loop while designing and tuning the generation and validation stages. -->
...
<!-- The concepts barely overlap, so the two halves together span a wide slice of finance. -->
...
<!-- Difficulty is a real axis, not a cosmetic one.  -->
...
<!-- Therefore we conclude that v1 failures are semantic, not syntactic:  -->
These results suggest that v1 failures are primarily semantic rather than syntactic: SQL errors are near zero, while wrong predicates and incomplete retrieval dominate, so models fail at data selection, not arithmetic. v2 moves to tool use: wrong-tool selection rises under distractors, and 
<!-- the open model exhausts its step budget on a quarter of its failures.  -->
...
<!-- Two models at the same accuracy fail for different reasons, which accuracy alone hides.  -->
...
<!-- We thank the reviewer for their interesting questions, which we address below with direct measurements. -->
...
<!-- Finance only supplies verifiable semantics and an executable ground truth, so FinOpsBench measures general agentic competence as much as financial knowledge. -->
...
<!-- The headroom is real and the difficulty is controllable,  -->
...
<!-- The clearest way to see what FinOpsBench adds is to run the same model on open finance benchmarks and on ours.  -->
...
<!-- FinOpsBench measures the retrieval-planning and tool-use capability they cannot reach. -->
...
<!-- , and the contrast is what sets FinOpsBench apart.  -->
...
<!-- A reproducible, controllable agentic benchmark with executable answers is exactly what the others cannot offer. -->
...
<!-- The benchmark already reports more than a single accuracy number,  -->
...
<!-- , and give a human reference point rather than a purely automatic one: -->
...
<!-- On v1 the failures are semantic, not syntactic:  -->
...
<!-- So the judgement is highly correlated with the human judge with domain knowledge. -->
...
In addition, the scoring verdict does not depend on which model does the judging. We re-scored the same 170 human-labelled v1 items with three further judges from different vendors, using the identical evaluation prompt (one representative run per judge):

| Judge model | agreement with human | Cohen's κ vs human |
|---|---|---|
| o4-mini (paper's judge) | 84.1% | 0.67 |
| Claude-Sonnet-4.5 | 78.2% | 0.51 |
| Gemini-2.5-Flash | 81.2% | 0.58 |
| DeepSeek-V3 | 80.0% | 0.55 |

All four judges agree with the human at 78 to 84% and with one another at Fleiss κ = 0.69, indicating that the verdict reflects the answer rather than the choice of judge model.
...
<!-- Nor is the dependence locked in:  -->
...
We thank the reviewer for their detailed review. Several concerns revolve around design choices (ground truth, the FinQA derivation, contamination), which we address with direct experiments. We also clarify several points regarding the evaluation harness and model selection.
<!-- a few rest on factual points about the harness and model choices that we are glad to clarify.  -->
<!-- We take every point seriously and answer them all below. -->
...
<!-- - **The FinQA derivation is a controlled intervention that -->
...
<!-- We hold the *question content* fixed (human-authored, community-familiar, with known static-setting numbers) and change only the *access mode* from reading to tool use.  -->
...
- **smolagents is current and actively maintained.** It is a current Hugging Face library (2025 release), not a deprecated framework. We chose it precisely because it is a *minimal* harness.
<!-- less scaffolding means *less* framework noise, not more. -->
...
This is an important concern, which we directly evaluate in three ways:
...
<!-- Memorization would *scale* with capability and training exposure; a flat floor is the signature of the residual scenario-text figures, not recall. -->

**(2) Access-mode dependence (200-item ladder).** 
<!-- A memorized answer would be emitted regardless of access mode.  -->
...
<!-- — it cannot appear in any training corpus, so contamination there is impossible.  -->
