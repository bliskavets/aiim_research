...
Part of this is already in the paper: Appendix C gives per-item statistics (query and prompt lengths, table and row counts, tool calls per item), Appendix D example queries, and Appendix G the v1 category distribution. We add one measurement per axis below.
...
Their SQL structure goes well beyond single-table lookups:
...
v1 concentrates on accounts payable, controls and variance; v2 on financial-statement ratios.
...
For "Which invoices have duplicate payment records, and what is the total overpaid?" GPT-4.1 aggregated at the invoice level instead of detecting repeated identical payments, so it reported only Invoice 4 at $0.01 and missed the real duplicates on Invoices 1, 3 and 5 of $600, $200 and $450.
