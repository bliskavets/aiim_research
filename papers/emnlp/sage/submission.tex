...
AlpacaEval reward dynamics across epochs and the compute-allocation ablation (\(N\), \(T\) sweep on MATH-500) are in Appendix~\ref{app:ablation-more}. Temperature ablations (\cref{fig:temp-ablation} in the Appendix) confirm that lower judging temperatures improves ranking quality while higher generation temperatures (\(\approx 0.9\)) maximizes solution discovery.
...
\noindent We conduct additional ablations on reward-model agreement and refinement geometry visualizations in section Appendix~\ref{app:refinement_geometry}. We also conduct an additional analysis of how the prompt's instructions to produce a more confident estimates affect the model's judgement accuracy. The ablation details can be found in Appendix~\ref{sec:app-prompt-calibration}.
...
To verify that \method{} remains effective for reasoning (thinking) models, we evaluate on AIME~2026, a competition-level mathematics benchmark with fewer than 100 instances and therefore well suited for inference-time compute scaling. Both models run in their native reasoning mode with a maximum sequence length of 32,768 tokens. Results are shown in \Cref{tab:aime2026}. On Qwen3-8B, \method{} outperforms Best-of-\(N\) with Skywork-Reward-V2-Llama-3.1-8B \citep{liu2025skywork} while using no external reward model. On Qwen3.5-4B-Thinking, \method{} further improves over the greedy baseline by seven points.
...
% We evaluate \method{} at two additional scales beyond the 8B models used in the main experiments: Qwen3-1.7B and Qwen3-32B-FP8, on a held-out MATH-500 subset (seed=42). Results, including wall-clock times, are shown in \Cref{tab:small-large-models}. \method{} consistently outperforms Best-of-\(N\) at both fixed candidate count and matched wall-clock time across all three scales. For Qwen3-1.7B, \method{} with two optimization epochs reaches 0.88, surpassing the best BoN baseline (Skywork-Reward-V2-Llama-3.1-8B, $N=70$: 0.86) while using only the 1.7B generator. For Qwen3-32B-FP8, \method{} reaches 0.97, outperforming BoN with Skywork-Reward-V2-Llama-3.1-8B ($N=7$: 0.94) from a strong 0.80 greedy baseline.
...
To assess generalization beyond \cref{sec:experiments}, we evaluate \method{} on IFEval, a strict instruction-following benchmark, and on a subset of MMLU-Pro STEM covering multi-step reasoning in science and engineering. Results with Qwen3-8B-FP8 are shown in \Cref{tab:additional-benchmarks}. Compared to only 4-point gains from Best-of-\(N\), \method{} improves IFEval prompt accuracy by 28 points and MMLU-Pro STEM accuracy by 20 points, indicating that iterative self-refinement generalizes beyond mathematical problem-solving to instruction-following and broader STEM reasoning.
