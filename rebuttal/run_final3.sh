#!/usr/bin/env bash
# Final 3 jobs, run with reduced gpu-memory-utilization to work around ~33GB stuck in a
# zombie CUDA context (only ~109GB of 143 usable). Servers verify the served model id.
#  - b4_math : Qwen3-1.7B @0.30, 32768
#  - E9      : BoN + Skywork RM on 8B @0.45 (RM loads into free GPU via transformers)
#  - C1      : Qwen3-32B-FP8 @0.65 (waits for the background download to finish first)
set -uo pipefail
...
if start_server "Qwen/Qwen3-1.7B-FP8" 0.30 32768; then SMALL="Qwen/Qwen3-1.7B-FP8";
else start_server "Qwen/Qwen3-1.7B" 0.30 32768 && SMALL="Qwen/Qwen3-1.7B" || SMALL=""; fi
...
# E9 (8B + Skywork RM)
start_server "Qwen/Qwen3-8B-FP8" 0.45 32768 || exit 1
...
# C1 (32B) - wait for background download
log "waiting for 32B download"
for i in $(seq 1 240); do grep -q "DL32_DONE_0" /root/aiim/external/qwen32b_dl.log 2>/dev/null && break; sleep 60; done
if start_server "Qwen/Qwen3-32B-FP8" 0.65 32768; then
